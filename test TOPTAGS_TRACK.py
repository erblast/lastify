...
# test load_toptags_track function independent of thread

if test_function:
    load_toptags_track(db_path, db_lock, call_cont, out, n_entries=100)

if test_thread:
    cond_event=threading.Event()
    thr_toptags_track=TOPTAGS_TRACK(db_path,'toptags_track', call_cont, out_cont, db_lock, 100, cond_event)
    cond_event.set()
...
if close:
    thr_toptags_track.signal_event.wait()
    out_cont.stop()
    call_cont.stop()
    con.close()
