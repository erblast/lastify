...
if test_function:
    if THREAD==False:
        load_spotifyID_track(db_path, db_lock, call_cont, out, n_entries=100)

if test_thread:
    thr_spotifyID_track=SPOTIFYID_TRACK(db_path,'spotifyID_track', call_cont, out_cont, db_lock, 100)
...
if close:
    thr_spotifyID_track.signal_event.wait()
    out_cont.stop()
    call_cont.stop()
